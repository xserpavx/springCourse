        ...
        $(function() {
                                    ...
                                    $.ajax({
                                        ...
                                        complete: function(result) {
                                            if (result.status === 200) {
                                                var data = '';
                                                result.responseJSON.forEach(function(book) {
                                                    data += Card().bookTemplate(book);
                                                });
                                                var $Cards = $('.Cards_refresh');
                                                $Middle.find('.Middle-searchHeader').text('Найдено ' + result.length() + ' книг' + (result.length() % 10 < 10 && result.length() % 10 > 20 ? (result.length() % 10 === 1 ? 'a' : (result.length() % 10 > 1 && result.length() % 10 < 5 ? 'и' : '')) : ''))
                            ...
                            if ($this.attr("name") === 'typeauth') {
                                switch ($this.val()) {
                                    case 'phonetype':
                                        ...
                                        $mail.addClass('form-input_hide').prop('disabled', 1);
                                        $sendButton.data('type', 'phone');
                                        break;
                                    case 'mailtype':
                                        $mail.removeClass('form-input_hide').prop('disabled', false);
                                        $phone.addClass('form-input_hide').prop('disabled', 1);
                    ...
                    init: function() {
                        ...
                        $btnSend.on('click', function(e) {
                            ...
                            if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                                ...
                                // HELP Если на строке 3478 поставить breakpoint то обновление страниц после post запросов начинает нормально работать
                        ...
                        $avatarfile.change(function() {
                            ...
                            if (readURL(this)) {
                                ...
                                if (!$thisAvatar.next('.form-error').length) {
                                    $thisAvatar.find('input[type="file"]').data('errorinput', 1);
            ...
            var Transactions = function() {
                var transactionTemplate = function(transaction) {
                    ...
                    return '<tr>\n' + '                      <td>' + date.getDate() + ' ' + (month[date.getMonth()]) + ' ' + date.getFullYear() + ' ' + date.getHours() + ':' + date.getMinutes() + '                      </td>\n' + '                      <td><span class="Transactions_textSuccess">\n' + '                          <strong>' + transaction.value + ' р.\n' + '                          </strong></span>\n' + '                      </td>\n' + '                      <td>' + transaction.description + '                      </td>\n' + '                    </tr>';
                }
                function getData(address, data) {
                    var scroll = $(window).scrollTop();
                    $.ajax({
